...
Future<void> onStart(ServiceInstance service) async {
  ...
  while (true) {
    try {
      final pendingNotifs = await NativeNotifications.readAll();
      print("Fetched ${pendingNotifs.length} notifications from file");
      ...
      final location = await LocationModule.getLocation().catchError((_) => null);
      ...
      if (response.statusCode == 200) {
        print("Uploaded ${pendingNotifs.length} notifications successfully.");
        await NativeNotifications.clearIfTooLarge();
      } else {
